# Probe 03: instrumented run on the CURRENT build.
#   C2: devpart OFF  <- the configuration that currently produces CORRECT output
#   A3: devpart ON + PREFETCH ON  (contrast)
# The new "inputs breakdown" counters cover the shared host-weights path, so both
# configurations report the same sub-phases.
...
Write-Host "=== C2: devpart OFF (correct output baseline) ==="
Set-Common
Remove-Item Env:\LLAMA_MOE_DEVPART -ErrorAction SilentlyContinue
$env:LLAMA_MOE_CACHE_STATS='stats-C2-ref.csv'
& $exe -m $model @flags > dsh-C2-out.txt 2> dsh-C2-err.txt
Write-Host "C2 exit=$LASTEXITCODE"

Write-Host "=== A3: devpart ON + PREFETCH ON ==="
Set-Common
$env:LLAMA_MOE_DEVPART='1'
$env:LLAMA_MOE_CACHE_STATS='stats-A3-devpart.csv'
& $exe -m $model @flags > dsh-A3-out.txt 2> dsh-A3-err.txt
Write-Host "A3 exit=$LASTEXITCODE"

Write-Host "=== probe 03 done ==="
