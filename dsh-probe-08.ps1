...
Set-Common
$env:LLAMA_MOE_CACHE_STATS='stats-H1-ref.csv'
& $exe -m $model @flags > dsh-H1-out.txt 2> dsh-H1-err.txt
Write-Host "F1 exit=$LASTEXITCODE"

Write-Host "=== F2: devpart ON ==="
Set-Common
$env:LLAMA_MOE_DEVPART='1'
$env:LLAMA_MOE_CACHE_STATS='stats-H2-devpart.csv'
& $exe -m $model @flags > dsh-H2-out.txt 2> dsh-H2-err.txt
