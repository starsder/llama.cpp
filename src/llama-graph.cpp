...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    static const bool moe_split = []() {
        ...
        // FIXED (handoff 6.30/6.32): the host split used to feed stale ids/weights into the
        // MoE GEMMs because the five host leaves are inputs of the same split and their copies
        // could be queued before the partition hook wrote them.  The hook now re-issues those
        // copies after filling, so any non-zero value is safe again.
