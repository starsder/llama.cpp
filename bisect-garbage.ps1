...
$base = @(
  ...
  "-p", "The capital of France is", "-n", "16", "--temp", "0", "-st",
...
# A: join=0, no PLE, insert on
$env:LLAMA_MOE_PREDICT_SMOE="1"; $env:LLAMA_MOE_PREFETCH_JOIN="0"; $env:LLAMA_MOE_INSERT_ON_MISS="1"
$env:LLAMA_PLE_CACHE_MIB="0"; $env:LLAMA_PLE_GPU_CACHE_MIB="0"
& build-ple-trace-mrs\bin\llama-cli.exe @base *> bisect-A.txt
Write-Output "A done"

# B: join=1, PLE on, insert on
$env:LLAMA_MOE_PREFETCH_JOIN="1"; $env:LLAMA_PLE_CACHE_MIB="2048"; $env:LLAMA_PLE_GPU_CACHE_MIB="1024"
& build-ple-trace-mrs\bin\llama-cli.exe @base *> bisect-B.txt
Write-Output "B done"

# C: join=0, PLE on, insert on
$env:LLAMA_MOE_PREFETCH_JOIN="0"
& build-ple-trace-mrs\bin\llama-cli.exe @base *> bisect-C.txt
Write-Output "C done"
