...
Set-Common
$env:LLAMA_MOE_CACHE_STATS='stats-F1-ref.csv'
& $exe -m $model @flags > dsh-F1-out.txt 2> dsh-F1-err.txt
Write-Host "F1 exit=$LASTEXITCODE"

Write-Host "=== F2: devpart ON ==="
Set-Common
$env:LLAMA_MOE_DEVPART='1'
$env:LLAMA_MOE_CACHE_STATS='stats-F2-devpart.csv'
& $exe -m $model @flags > dsh-F2-out.txt 2> dsh-F2-err.txt
