# 历史来源快照：smoe-nk-degradation-plan.md
...
- 原来源：`docs/smoe-nk-degradation-plan.md`（本地工作区快照，不能假定与已发布 master 相同）。
- 原文件 SHA-256：`31f0d2018fedc234c8568959ab452c80ce183f7c15bcd9905e9575afc94c0592`。
...
**采集方式**:专属插桩检出 `OTHER_SOURCE_TREES\qwen4exp-smoe-trace`(`LLAMA_TRACE_SMOE=<目录>`),
...
`OTHER_SOURCE_TREES\qwen4exp-smoe-trace\ggml\src\ggml-backend.cpp:1607-1610`
...
**(a) 补 dump(推荐,最省事)**
在 `OTHER_SOURCE_TREES\qwen4exp-smoe-trace` 里两处改动:
```cpp
...
- **48 个 gate 矩阵**:`LOCAL_MODELS\qwen38\traces\gate\blk.N.ffn_gate_inp.weight.npy`
  (从 GGUF 抽出,已归一为 `[n_expert=512, n_embd=2560]`,F32)
  抽取脚本:`LOCAL_MODELS\qwen38\traces\extract_gate.py`
...
数据:`LOCAL_MODELS\qwen38\traces\standard-smoe-8-20260911`(8 个 prompt,新增 `ffn_moe_input`)
...
模拟器:`LOCAL_MODELS\qwen38\traces\simulate_smoe_nk.py`
