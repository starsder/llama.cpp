...
- 原来源：`handoff.md`（本地工作区快照，不能假定与已发布 master 相同）。
- 原文件 SHA-256：`fd1794c5a8e84420b69ca521cade6f78253ea823b7b49ef46aea5a94510bd6e6`。
...
更新: 2026-09-12 | 工程: `SOURCE_TREE` | 构建: `build-ple-trace-mrs`
...
**clean 线实验已放弃**：`OTHER_SOURCE_TREES/llama-qwen4exp-clean`（worktree @ `b76199698`）里留了一份 `clear/` 迁移快照
...
- 正确跑法（已实测 ✓）：
  `llama-speculative-simple.exe -m <IQ3_XXS 00001> -md LOCAL_MODELS/qwen38/MTP/mtp-...-Q4_K_M.gguf
...
**§6.30 补充：SMoE 前瞻距离实测（`LLAMA_MOE_SMOE_AHEAD`，8k/q8_0/64 槽，SPLIT=1）**

| ahead | r1 | r2 | r3 | 命中率 | gen |
|---|---|---|---|---|---|
| 1 | 58.5% | 49.7 | 43.6 | 68.2% | 17.2 |
| 2（原默认） | **84.7%** | 76.5 | 70.2 | 77.4% | 18.1 |
| **3** | 80.9% | 67.9 | 60.7 | **85.4%** | 19.2 |
| **4** | 78.6% | 68.7 | 59.5 | **85.7%** | **19.9** |
| 6 | 56.1% | 46.2 | 39.7 | 80.6% | 18.5 |

- **最优点在 3–4 层**（相对默认 2：命中 +8 点、+1.8 t/s）。关键洞察：**命中率不是由 r1 单项决定的** ——
  ahead=2 的 r1 最高（84.7%）却命中最低（77.4%），而 3–4 层 r1 略降但**提前量**足够让预取真正完成，
  于是命中反升。即"越远时间越多"的收益在**时间**侧兑现，代价是 r1 下降，交点 ≈3–4。
- `ahead=1` 的 r1（58.5%）显著低于 2，怀疑侧图在 1 层间隔下的代理输入有 off-by-one（待查）。
- 这组是在**带 bug 的 SPLIT=1** 下测的 ⇒ 趋势可靠、绝对数字待修复后复核。

**建议**：`LLAMA_MOE_SMOE_AHEAD=4`（可用 env 立即生效）；参数别手工扫 ⇒ 后续接回项目既有的**自适应搜索**
（`MAutotune` 那套：按命中率/速度在线调 ahead、准入等）。

**测量装置注意事项（本轮踩过的坑）**
- llama-cli 走聊天路径会起**内部 llama-server**；`--no-mmap` 下外层可能仍持有模型句柄，
  内层加载偶发失败并报 "the server exited before becoming ready"/No such file ⇒ 需**重试**（脚本已加）。
- 检测退化类 bug **必须不要 `--ignore-eos`**（否则退化尾部把 bug 掩盖掉，见 §6.30 开头）。
- 复现用的 prompt/长度要与参考一致，否则长度阈值不可比。


### 6.31 2026-09-13 夜：可部署性矩阵 + 安全锁（用户已睡，本轮自主收尾）

**可部署性矩阵（同一 rig 实测）**

| 配置 | 速度 | 正确性 | 结论 |
|---|---|---|---|
| 缓存 **关** | **9.2 t/s** | ✓ | 当前唯一可安全部署 |
| 缓存 **开 + SPLIT=1** | 22.0 t/s | ✗ **静默算错**（§6.30） | 不可部署 |
| 缓存 **开 + SPLIT=0** | **3.4–6.5 t/s** | ✓ | 比不用缓存还慢 —— 专家仍在 CPU 算，而权重被搬到显存 ⇒ 每次读专家变成 PCIe 拷贝 |
| 缓存 开 + SPLIT=1 + 0 槽（64 MiB） | 无输出 | ✗ | 独立的确定性 bug：0 槽时 split 不产出（本该退化成全 CPU） |

⇒ **缓存的全部收益依赖 split；split 不正确 ⇒ 当前缓存在任何配置下都不可部署。** 那 22 t/s 是假速度，
15.9 是长尾里的一次偏快样本（同配置另测 6.5/4.0/3.7/3.4 ⇒ 不可重复）。

**已上安全锁（本轮交付）**
- `src/llama-graph.cpp`：`LLAMA_MOE_SPLIT=1` **拒绝生效**并打印原因（指向本节）；
  调试需显式 `LLAMA_MOE_SPLIT=2`；`LLAMA_MOE_DEVPART=1` 仍允许（devpart 是另一条已验证正确的路径）。
- `tools-run.py` / `run-cur-ref.ps1`：默认改为 `LLAMA_MOE_SPLIT=0`（避免脚本产出静默错误的对照数据）。
- 复现/回归用例：`-p 'Write a short factual paragraph about the Eiffel Tower.' -n 160 --temp 0`（**不加** `--ignore-eos`）
  ⇒ 正确 = 751 字符；错误 = 27 字符（`<think>` 后即止）。三种长度 27/652/751 说明是**部分性竞态**。

**下一步（未做，需清醒时定夺）**
1. 修 split 竞态（`handoff 6.30` 的修复方向：按层/按 token 多份 leaf 缓冲、direct 一致性断言回退 gather、
   0 槽退化全 CPU）；修好后用上面的用例回归；
2. 再复测 `SMOE_AHEAD`（§6.30 补充的 3–4 最优是在**错误模式**下测的 ⇒ 趋势可信、绝对值待复测）；
...
### 6.33 检查项 1 结案：`SMOE_AHEAD=1` 的 r1 异常 = 非阻塞读回的"晚一拍"

**现象（修复 SPLIT 之后复测，仍然存在，故非 artifact）**：r1 在 ahead=1/2/3/4 = 61.2 / 86.0 / 79.5 / 72.3%
—— 2→3→4 平滑衰减，唯独 1 脱轨。

**定位**
- 归属偏移**正确**（`moe_cache_smoe_process`：`target_layer = pending.layer + s.smoe_ahead` ✓；4454 那处
  hardcoded `+1` 属于 fate/manifest 路径，本配置里 `predictor=0` 直接早退，不参与）；
- 交付统计四个 ahead 几乎相同（`deferred=11`、`predict≈11–12k`）⇒ 不是"丢预测"；
- **决定性实验**：把 SMoE 读回改成同步（`LLAMA_MOE_SMOE_NONBLOCK=0`）后，
  **ahead=1 的 r1 从 61.2% → 93.1%**（全场最高），而 ahead=2 基本不变（86.0 → 84.7）
  ⇒ **非阻塞读回会让预测被晚一拍消费**；`ahead=1` 的窗口只有 ~1 层，恰好整拍过期（用到的其实是
  上一层（L−1）的预测），而 `ahead≥2` 时晚一拍消费的仍是"目标为 L"的预测，故正常。
...
⇒ **速度上现默认仍最优**（均值 20.1 vs 19.1 / 18.3，且同步路径方差更大）⇒ **不改默认**。
...
### 6.36 混合类型（UD）对读取策略的影响：审计 + 运行期保险

**问题**：UD 量化一个文件里混了 `iq2_s` / `iq4_nl` / `iq3_s`，而三者**块大小不同**
（256/82、32/18、256/110）；缓存按"行/专家偏移"寻址，任何把行大小当常量的地方都会静默错位。

**审计结论：寻址是"逐张量几何"，不是"全局类型"，因此不会出问题**（`ggml/src/ggml-backend.cpp`）：

| 保障 | 位置 |
|---|---|
| 每专家字节 = **该张量自己的** `nb[2]` | 3107 |
| 块字节 = **该张量自己的** `ggml_type_size` | 3108 |
| 每分量起始按**自己的** `type_size` 对齐（下限 512） | 3206 |
| slot 步长按**所有分量 type_size 的 LCM** 对齐（保证 `nb[2]/type_size` 整除） | 3210-3218 |
| 专家数不一致 → **禁用缓存**（不继续猜） | 3200-3203 |
| 无任何硬编码类型常量（98/82/110/18/34/66 全文 0 处） | — |

代码注释本身已写明理由（"512 alone is not divisible by e.g. IQ3_XXS' 82"），说明这块被专门考虑过。

**实测核对**（本模型）：gate/up `iq2_s` 820B/行 × 640 = 524800 B，`% 82 == 0` ✓；
down `iq4_nl` 360B/行 × 2560 = 921600 B，`% 18 == 0` ✓；
bundle stride 实测 **2078208 B** = `11 × lcm(512, 82, 18)=11 × 188928` ⇒ 对齐浪费 **9728 B = 0.47%**。
新增 `tools-gguf-types.py --check`：1224 个张量全部 PASS（`ne[0] % block == 0`、每专家是块字节整数倍、`nbytes == expert*ne[2]`）。

**新增运行期保险**（`moe_cache_entry` 创建处）：若某张量的 `expert_size % type_size != 0`，
打印 `[MOE-CACHE] ... unsupported quant layout, disabling cache` 并禁用缓存 —— 把未来换量化时
可能出现的"静默错位"变成明确报错。验证：本模型跑 256 token 无任何误报，`slots= 97` 正常，20.1 t/s / 89.7%。

### 6.37 缓存压缩空间评估：**基本没有**（实测）

**1) 无损压缩 ≈ 0**（直接读 GGUF 张量原始字节，前 4 MiB 样本）：
...
⇒ 量化权重熵已饱和；连 11–12% 的 fp16 `d`/`scales` 字段本身也不可压。

**2) "元数据"只占 5.5–12.2%，且缩它=换格式**（`ggml-common.h` 块结构，QK_K=256/QK4_NL=32）：

| 张量 | 格式 | 块 | 元数据 | 占比 | 缩元数据的现成替代 | 折算到总预取字节 |
|---|---|---|---|---|---|---|
...
**3) 缓存自身布局浪费 0.47%**（见 6.36 实测 2078208 B）⇒ 无空间。

**4) 唯一有杠杆的"压缩" = 降低 bpw（换配方）**：down 占预取字节 **46.8%** 却是 **4.5 bpw**；
换成 `iq2_s`（2.5625 bpw）⇒ 总预取字节 **-20%**（43→34 ms，[INFERENCE] +2 t/s 量级），
且每 slot 小 20% ⇒ **97 → ~121 槽**，命中率再上一档。但这是**模型配方**决定（质量会变），
不是工程优化 —— 需要做一个 down=iq2_s/iq3_s 的 UD 变体才能测。
...
### 6.39 CPU 算子效率实测 + "4n/repass 拉满"的可行性评估（结论：不是当前的瓶颈）

**实测方法**：`tests/test-quantize-perf.cpp` 原先对没有 `from_float` 的类型（全部 i-quant）**直接跳过**，
所以 i-quant 从来测不到。已打补丁：`vec_dot_q` 对这类类型改用**原始字节填充**（i-quant 的 vec_dot 是
查表实现、数据无关，吞吐量有效）；`build-cli.bat` 现在接受 target 参数（`build-cli.bat test-quantize-perf`）。

**单核 vec_dot 实测（量化字节 GB/s，以及换算的权重速率）**

| 类型 | size=2560 | size=65536 | 权重速率 | bpw |
|---|---|---|---|---|
| **iq2_s**（gate/up，53% 预取字节） | 2.13 | **2.30** | **7.2 G/s** | 2.56 |
| **iq3_s**（blk.2 gate/up） | 1.82 | 2.00 | 5.8 G/s | 3.44 |
| iq4_nl（down，47%） | 12.19 | 14.88 | 26.4 G/s | 4.5 |
| q6_K | 16.86 | 24.93 | 30 G/s | 6.56 |
| q8_0 | 25.85 | **36.43** | 34 G/s | 8.5 |

⇒ **i-quant 的 CPU 内核比同类 4-bit 慢 6–15×**，且是**纯 kernel-bound**（DRAM 还有 10× 余量）。
原因：LUT + 位域拼接密集，AVX2 缺 `vpermb`/gather；上游只在 AVX512/NEON 上给了 repack 版本
（`q2_K_8x8_q8_K` 需 `ggml_cpu_has_avx512()`，`q5_K/q6_K` 仅 NEON），**x86 AVX2 只有 `IQ4_NL` 有 repack**。

**但对我们当前配置，这不是瓶颈**（实测 `CACHE_MIB=auto` vs `4096`，200 token，8k/q8_0）：

| 预算 | 槽 | 命中 | `cpu=` | `pre=` | total | gen |
|---|---|---|---|---|---|---|
| `auto` | 97 | 80.2% | **1.5 ms** | 49.3 ms | 63.3 ms | ~20 t/s |
| 4096 | 42 | 78.0% | **1.7 ms** | 48.0 ms | 62.0 ms | ~20 t/s |

⇒ CPU 半边只占 **2.4–2.7%**，且**与槽数几乎无关**（42 vs 97 槽只差 2 个百分点命中）。

**关键副产物 1：缓存需要的显存远少于预期** —— 42 槽（4096 MiB）就有 78% 命中
（vs 97 槽 80.2%）⇒ 256k+vision 场景不必为缓存留大显存，多出的 ~5 GB 可以给上下文。

**关键副产物 2：真正的瓶颈在主机的预取路径** —— 完整分解：
`total=63.3 | pre=49.3 | smoe=17.49 | gpu_queue=7.7 | ids_wait=4.1 | partition=2.73 | cpu=1.5`（ms，有重叠）。
`pre` 约 630 MB/token（按 §① 实测 0.073 ms/MB 反推）—— **是真实漏命中量（~180 MB/token）的 3.5×**，
即**准入策略超额预取**；且 49 ms / 630 MB ≈ 13 GB/s，是**主机侧拷贝/提交**的速度，不是 PCIe 上限。
⇒ 这里（而非 CPU 内核）才是"拉满效率"该动的地方：并行化/合并预取拷贝、按字节预算收紧准入。
[INFERENCE] 若 `pre` 能压到 ~20 ms，token 时间 63→~35 ms ⇒ ~28 t/s。

**CPU 内核在什么时候值得优化**：CPU-heavy 模式 —— 缓存关闭/很小、或 `--cpu-moe` 全 CPU 时
...
### 6.43 NXQ GPU KV 压缩接入

使用现有模型命令，保留 `--cpu-moe` 和 MoE cache 配置，只替换 KV 参数：
`-fa on -ctk nxq3_0 -ctv nxq2_0`，或两者均为 `nxq2_0`。不需要 `-nkvo`。
当前验证模型的 head_dim 为 256；NXQ 的量化块大小也是 256。

- 每块一个 fp16 scale，按 8 维选择整数偶数和候选或半整数候选；半整数陪集按参考
  NexusQuant 0.6.3 放宽奇偶限制，并用每组 1 bit 显式保存，不能再次取整丢失。
- `nxq3_0` 每块 102 B（3.1875 bit/元素）；`nxq2_0` 每块 70 B（2.1875 bit/元素）。
  这些数字包含 scale 和陪集标记。编码前把两个候选各自投影到本格式能存的坐标区间
  （整数陪集 `[-levels/2, levels/2-1]`、半整数陪集 `[-levels/2+1/2, levels/2-1/2]`）
  再比较距离；这是对参考 fake-quant 结果的定点近似，见 §6.44。
- CPU 编解码、点积、CUDA `SET_ROWS`、`F32↔NXQ`、`NXQ→F16`、连续/非连续解码均已接入。
  KV 沿用现有 Hadamard 旋转；日志实测 `attn_rot_k = 1`、`attn_rot_v = 1`。
- KV 在 CUDA0 保持压缩存储，注意力前显式解码为 F16，再调用已有 F16 Flash Attention。
  未新增直接读取 packed NXQ 的 FA 内核；有解码计算和 F16 临时缓冲开销。
  不把 NXQ 加入 CUDA FA 类型白名单，也不将注意力回退到 CPU。
- 修复了初始 CUDA 解码索引将 256 元素 block 当成 32 元素 block、漏写元素的问题；
  也修复了 scale 下溢到零时除零的问题。

验证：完整构建通过；GPU 多 block、不同 scale、乱序 SET_ROWS、F32/F16 解码及
非连续 head/token 视图转换与 CPU 数值一致（该样本 max error = 0，正负零视为相等）。
CPU 半整数陪集、零块、极小值 scale、点积回归均通过。
`test-quantize-fns` 将点积内核误差与量化失真分开测量；NXQ 项均通过，
但现有 TBQ3/TBQ4 的总量化误差分别 0.003214/0.002025，仍超过原阈值 0.002，未调宽阈值。

同一模型、GPU KV、`--cpu-moe`、MoE cache 2048 MiB、`-c 512 --chunks 1 -fa on`、
WikiText-2 首块的 PPL（均 exit 0；样本小，不能代表长上下文质量）：

| K / V | 主 KV buffer | PPL |
|---|---:|---:|
| q8_0 / q8_0 | 25.50 MiB | 2.3458 |
| nxq3_0 / nxq2_0 | 8.06 MiB | 2.8378 |
| nxq2_0 / nxq2_0 | 6.56 MiB | 2.7150 |

buffer 数字为该次 PPL 的 4-sequence 分配；不含独立 indexer KV、recurrent state 和 compute buffer。
当前结果证明压缩路径可用，不证明质量优于 q4/q8，也不证明 256k 性能。
旧无陪集标记格式的 NXQ 往返误差和 nkvo 实验不能作为本实现的质量证据。
本次原始运行日志：`LOCAL_EVIDENCE/nxq-quality-{q8,nxq32,nxq22}.log`、`LOCAL_EVIDENCE/nxq-gpu-cli.log`。

### 6.44 NexusQuant 对齐：参考真实链路、本次两处修改、两处结构性缺口

参考 `nexusquant_kv-0.6.3` 的**实际**链路（源码已与 PyPI sdist 逐字节核对）：

- keys：`inverse_rope` → Hadamard（**整个 head**：`hadamard_matrix(head_dim)`）→ E8 VQ →
  逆 Hadamard → `forward_rope`；
- values：Hadamard（整个 head）→ E8 VQ → 逆 Hadamard；
- `E8QuantizedCache.min_seq_for_quant=2`：**单 token 的 decode 写入保留 fp16**（KIVI 式残差），
  只有 prefill 块被量化；
- 存储：`round(lattice*2) → int8` + 时间差分 + zstd（**变码率**）；"3-bit" 是名义值。
  `quantize_perhead` 返回反量化后的浮点（fake quant），字节数由 `measure_compression` 另算。

本实现（`nxq3_0`/`nxq2_0`，定长 3.1875/2.1875 bpw）对照：

| 项 | 参考 | 本实现 | 状态 |
|---|---|---|---|
| E8 候选/偶数和修/陪集判据/每头 fp16 scale | — | 同 | ✅ 99.98% 坐标一致 |
| 坐标取值域 | clamp 到 `[-levels/2, levels/2]`（整数 9 值、半整数 10 值） | 3 bit 只放得下 8 值 | ⚠️ 见下 |
| K 的量化域 | **先去 RoPE** | 带 RoPE 量化 | ❌ 结构性 |
| V 旋转 | 整个 head | 整个 head（本次改） | ✅ |
| decode 残差 | 单 token 写入 fp16 | 全部量化 | ❌ 未实现 |
| 存储 | int8+delta+zstd | 定长 ggml 类型 | 设计选择 |

**本次改的两处**

1. **编码器改为"先投影再选"**（`ggml-nexusq.c` 与 `ggml-cuda/cpy-utils.cuh` 同步改）：
   两个候选各自投影到本格式能存的区间（整数 `[-maxq, maxq-1]`、半整数 `[-maxq+0.5, maxq-0.5]`）
   后再比距离，取代"先按未投影距离选、再饱和"。旧做法让最大的那个坐标稳定多出 1 步误差。
   证据（同一输入，两侧都用 fp16 scale）：
   - `test-quantize-fns` 绝对量化误差：3-bit 0.002881 → **0.002367**，2-bit 0.006117 → **0.004857**；
   - 对参考的归一化 MSE 比：高斯 1.037→**1.016**(3b) / 1.049→**1.020**(2b)；
     uniform 1.257→1.095 / 1.475→1.135；sine 5.15→1.83 / 8.02→2.40；
   - 重编译的 CPU 内核与设计稿 numpy 复刻逐元素全等（6 组用例 0 差异）。
   注意"与参考坐标一致率"从 99.64% 掉到 97.80%：这是**故意**在端点上用 1 个半步的偏离
   换掉原来 2 个半步的偏离。一致率不是质量指标，MSE 才是。

2. **V 旋转改为整个 head**（`build_input_v_rot` 采用与 K 相同的规则；本模型 = 256，
   参考对 V 也用整个 head）。A/B（`tools-kv-matrix.py kld`，8×512 chunk，对 f16 base）：

   | KV | V rot=64（旧） | V rot=256（新，已保留） |
   |---|---|---|
   | q8_0/q8_0 | 0.030848 / 99.9% 1.203 | 0.033869 / 99.9% 0.913 |
   | nxq3_0/nxq2_0 | 0.266610 / 99.9% 6.917 | **0.256936 / 99.9% 4.700** |

   `q8_0@64 = 0.0308` 与 §6.42 记录一致（协议可比）。NXQ 均值 KLD −3.6%、尾部 −32% ⇒ 保留新规则。
   ⚠️ 该规则影响**所有**量化 KV 类型：q8_0 均值 KLD 0.0308→0.0339（尾部 1.203→0.913）。
   `test-backend-ops` 新增 NXQ 用例：`CPY` 8 例（F32→NXQ 连续/乱序、NXQ→F32）+ `SET_ROWS` 4 例
   （KV 写入路径）全部 OK ⇒ **CPU 与 CUDA 编码结果一致**（该测试对 NXQ 的容差是 1e-6 nmse）。
   注：quantized→F16 的 `cpy` 在 CPU 后端本就不支持（`ggml-cpu/ops.cpp` 只实现 quantized→F32），
   所以没有这条用例；NXQ→F16 是 CUDA 侧读路径。

**未对齐的两个结构性缺口（是缺口，不是 bug）**

> 这两条已在 §6.45 用 fp16 fake-quant 复现路径**实测**：K 链（去 RoPE）在本地设置下
> 没有可测收益（0.2607 vs 0.2561，噪声内），打包也没有可测代价（0.2569）。所以
> "为 K 链写读取时 RoPE 的 fused kernel"**不值得做**；§6.45 列出的"边界层保护"才是下一步。

1. **K 带 RoPE 量化**：参考在去 RoPE 的域里做 E8；我们的 cache 存的是已 RoPE 的 K，
   旋转与量化都发生在该域。要按参考顺序做，需要**读取时逐 token 施加 RoPE**，
   即"直接读 packed NXQ + 位置表"的 fused FA kernel；当前设计是 `NXQ → F16 → 现成 F16 FA`
   （`ggml_cast` 会把整层 cache 物化成 F16）。在图上用 rope 覆盖整个 cache 是
   O(n_ctx)/token/layer，256k 下不可接受。
2. **没有 decode fp16 残差**：llama.cpp 的 KV buffer 只有单一类型，做不到"prefill 量化、
   单 token 写入全精度"；参考把这条列为质量关键项。

**当前质量定位**（同协议、对 f16 base、8×512 chunk；q4_0/tbq3_0 行是 §6.42 用 V rot=64 测的）：

| KV | 名义 bpw(K+V) | Mean KLD | 99.9% KLD |
|---|---|---|---|
| q8_0 | 17 | 0.0339 | 0.913 |
| q4_0 | 9 | 0.057 | — |
| tbq3_0 | 6.375 | 0.116 | — |
| nxq3_0/nxq2_0 | 5.375 | 0.257 | 4.700 |
| nxq2_0/nxq2_0 | 4.375 | 0.320 | 6.560 |

⇒ **NXQ 目前在"每比特质量"上落后**（5.375 bpw 的 0.257 比 6.375 bpw 的 tbq3_0 的 0.116 差 2.2×）。
两个结构性缺口是首要嫌疑；端点那 ~3% MSE 不是主因。

原始日志：`/tmp/nxq-align-{test,test-v,kld,kld-v64,final-build}.log`、`/tmp/nxq-tbo-{cpy2,sr}.log`、
`/tmp/align-edits.diff`。

### 6.45 NexusQuant 保真复现（fp16 fake-quant 路径）与结论

**问题**：§6.44 的实现缺 K 的去 RoPE 链和 decode 残差，所以它测出来的 KLD **不能**用来评价
NexusQuant 方法本身。参考的**运行时** cache（`E8QuantizedCache`）其实存的是 fp16——
它把去 RoPE→Hadamard→E8→逆 Hadamard→再 RoPE 全做完后存反量化结果（fake quant）；
int8+时间差分+zstd 只是 `measure_compression` 的**离线记账**，不是运行时格式。
因此"复现方法"= 在 f16 cache 上做同一条链。

**实现**：`LLAMA_KV_E8_FAKEQUANT`（0=关，1=参考链，2=同链但不做去 RoPE）。
链在 `models/qwen4exp.cpp::build_layer_attn` 内用**现有算子**表达，cache 保持 f16：
```
k: R(-p) → H → cast(NXQ3_0) → cast(F32) → H → R(p)      (mode 2 去掉两端的 rope)
v:          H → cast(NXQ2_0) → cast(F32) → H
```
`ggml_cast(x, NXQ3_0)` 就是量化 cpy（块 = 一个 256 维 head），所以链是 4 个已有算子。
进入该模式时 `attn_rot_k/v` 被强制打开（只为拿到 Hadamard 矩阵），模型侧禁用原来的
"写前旋转/读后反旋转"（`llama-graph.cpp::build_attn` 与 `build_attn_qsa` 均按模式+arch 门控）。
新增 `build_inp_pos_neg()`（`llm_graph_input_pos` 加 `negate`）提供 R(-p)。

**结果**（同协议：wikitext-2、8×512 chunk、对 f16 base、`-c 512`、K3V2）：

| 变体 | Mean KLD | 99.9% KLD | PPL(Q)/PPL(base) |
|---|---|---|---|
| q8_0（打包，8.5 bpw/张量） | 0.0339 | 0.913 | — |
| **参考链（mode 1，去 RoPE）** | **0.2607 ± 0.0139** | 5.612 | 1.1096 ± 0.0249 |
| **同链但不去 RoPE（mode 2）** | **0.2561 ± 0.0147** | 7.918 | 1.1062 |
| 本实现打包 nxq3_0/nxq2_0（§6.44） | 0.2569 | 4.700 | — |
| 本实现打包 nxq2_0/nxq2_0 | 0.3204 | 6.560 | — |

**结论**：
1. **去 RoPE 这一步在本地设置下没有可测收益**：0.2607 vs 0.2561，差 0.005 ± 0.020（噪声内）。
   ⇒ §6.44 里"要补 K 链（读取时 RoPE 的 fused kernel）"这条**不值得做**；先把钱花在别处。
2. **打包没有代价**：参考语义（fp16 存储）0.2561–0.2607，本实现打包 0.2569——三者一致。
   端点那 ~3% MSE（§6.44）在这个协议下不可见。
3. 因此 KLD 0.26 是**这个方法+这套配置在本协议下的性质**，不是我们实现的劣化：
   它比 q8_0 差 7.6×；与 tbq3_0（0.116，K=V=3bit=6.375 bpw）相比，NXQ 用 5.375 bpw 得到 0.257。
4. 仍未对齐、且有量级的只剩两项：
   - **边界层保护**：参考 README 明确写 Qwen 系需要 `protect_boundary=2`（首尾各 2 层不压缩），
     本实验**没有做**（48 层全压）。fake-quant 路径里加"前 2/后 2 层跳过 round-trip"即可 A/B，
     属于**下一步最该测**的一项。
   - **decode fp16 残差**（`min_seq_for_quant`）：只影响逐 token 生成；本协议按块写入，
     残差不参与，所以上面的数字不受影响。
5. 存储率仍是替代方案（参考为 int8+delta+zstd 变码率；本实现定长 3.1875/2.1875 bpw，
   与参考实测的 ~3.1 bit/dim 同量级）——这是**有意的替换**，需在报告里明确。

复现命令：`LLAMA_KV_E8_FAKEQUANT=1 python tools-kv-matrix.py kld --types f16/f16`
（mode 2 用 `=2`；mode 0 = 纯 f16 基线，99.9% KLD 4.2e-5）。日志：`/tmp/e8-kld-all.log`、
`/tmp/e8-m1b.log`、`/tmp/e8-m2b.log`。

**边界层保护（参考 README 要求 Qwen 用 `protect_boundary=2`）**：`LLAMA_KV_E8_PROTECT=N`
让首尾各 N 层跳过 round-trip（保持 f16）。mode 1 下：

| protect | 量化层数 | Mean KLD | 99.9% KLD | PPL(Q)/PPL(base) |
|---|---|---|---|---|
| 0 | 48/48 | 0.2607 ± 0.0139 | 5.612 | 1.1096 |
| 2 | 44/48 | 0.2526 ± 0.0139 | 5.999 | 1.1042 |
| 4 | 40/48 | 0.2406 ± 0.0132 | 6.260 | 1.0886 |

收益≈**线性**（每保护一层 ≈ −0.005 KLD，≈ −2%），没有"边界层特别脆弱"的迹象——
即这个模型上边界层不比中间层更该保护（参考的 `protect_boundary="auto"` 判据是
"边界层 key 幅度 >2× 中间层"，这里大概不成立）。按这个斜率，把 KLD 从 0.26 压到
q8_0 的 0.034 需要保护掉 ~80% 的层——等于放弃压缩。**边界保护不是解法。**

**最终判断**：在本地协议下，**参考方法本身**（fp16 fake-quant、整 head Hadamard、K3V2、
含/不含去 RoPE、含边界保护）的 Mean KLD 落在 **0.24–0.26**，而 q8_0 是 0.0339。
本实现打包版 0.2569 —— **与参考语义一致（差异在噪声内）**。因此：
- 之前的"我们比 tbq3_0 差 2.2×、每比特质量落后"**不是实现缺陷**，是方法+配置在此协议下的性质；
- 反过来，若要在同样的 5.4 bpw 下拿到 q4_0/q8_0 级别的 KLD，需要的不是"更忠实的复现"，
  而是**改量化方案本身**（更多比特、按层/按头自适应比特、或干脆换回 q4_0/q8_0）；
- 复现的剩余价值在于：它给了我们一个**可对照的参照实现**（`LLAMA_KV_E8_FAKEQUANT`），
  任何后续改动都能和"参考语义"直接比 KLD，而不必猜。
...
模型 `LOCAL_MODELS\qwen38\unsloth-iq3-xxs\UD-IQ3_XXS\Qwen3.8-Flash-Next-UD-IQ3_XXS-00001-of-00003.gguf`，
