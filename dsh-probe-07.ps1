...
Set-Common
$env:LLAMA_MOE_CACHE_STATS='stats-G1-ref.csv'
& $exe -m $model @flags > dsh-G1-out.txt 2> dsh-G1-err.txt
Write-Host "F1 exit=$LASTEXITCODE"

Write-Host "=== F2: devpart ON ==="
Set-Common
$env:LLAMA_MOE_DEVPART='1'
$env:LLAMA_MOE_CACHE_STATS='stats-G2-devpart.csv'
& $exe -m $model @flags > dsh-G2-out.txt 2> dsh-G2-err.txt
