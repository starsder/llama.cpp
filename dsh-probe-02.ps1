# Probe 02: isolate config A (devpart ON + PREFETCH ON). Run twice on the same
# build to check whether the mid-generation death is deterministic. Records exit codes.
...
$env:LLAMA_MOE_DIRECT_READ='1'; $env:LLAMA_MOE_DEVPART='1'; $env:LLAMA_MOE_PREFETCH='1'
...
foreach ($r in 1..2) {
    $env:LLAMA_MOE_CACHE_STATS="stats-A2-run$r.csv"
    Write-Host "=== A run $r ==="
    & $exe -m $model @flags > "dsh-A2-run$r-out.txt" 2> "dsh-A2-run$r-err.txt"
    Write-Host "A run $r exit=$LASTEXITCODE"
}
Write-Host "=== probe 02 done ==="
